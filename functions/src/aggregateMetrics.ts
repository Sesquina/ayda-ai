...
export const aggregateOrgMetrics = onSchedule("every 24 hours", async () => {
  ...
  const trialLogsSnap = await db.collection("trial_logs").get();
  trialLogsSnap.forEach((doc) => {
    ...
    const bucket = orgMetrics.get(monthKey)!;
    bucket.trialMatches += data.matchesReturned ?? 0;
  });
          ...
          .set(
            {
              ...
              trialMatches: bucket.trialMatches,
              updatedAt: Date.now(),
            },
            { merge: true },
          );
